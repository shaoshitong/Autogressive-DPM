...
def main(args):
    ...
    for x, y in loader:
        x = x.to(device)
        if args.ten_crop:
            x = einops.rearrange(x,"a b c h w -> (a b) c h w")
            x_all = x
            num_aug = 10
        else:
            x_flip = torch.flip(x, dims=[-1])
            x_all = torch.cat([x, x_flip])
            x_all = einops.rearrange(x_all,"a b c h w -> (a b) c h w")
            num_aug = 2
            
        y = y.to(device)
        with torch.no_grad():
            latent_code = vae_model.encode(x).latent_dist.sample().mul_(0.18215)
        codes = latent_code.reshape(x.shape[0], -1) # [1, 10, 32x32x4]
