...
class VectorQuantizer(nn.Module):
    ...
    def forward(self, z):
        ...
        z_q = embedding[min_encoding_indices].view(z.shape) # [(bhw) c]
        perplexity = None
        min_encodings = None
        vq_loss = None
        commit_loss = None
        ...
        if self.training:
            vq_loss = torch.mean((z_q - z.detach()) ** 2) # z_q is the argmin embedding
            commit_loss = self.beta * torch.mean((z_q.detach() - z) ** 2) 
            entropy_loss = self.entropy_loss_ratio * compute_entropy_loss(-d)

        # preserve gradients
        z_q = z + (z_q - z).detach() # gradient estimator
